...
package object rx {
  ...
  implicit class GenericOps[T](val node: Node[T]) extends AnyVal {
    ...
    def filter(f: T => Boolean)(implicit ownerCtx: Ctx.Owner, dataCtx: Ctx.Data): Rx[T] = macro Operators.filtered[T,T]

    def fold[V](start: V)(f: ((Id[V], Id[T]) => Id[V]))(implicit ownerCtx: Ctx.Owner, dataCtx: Ctx.Data): Rx[V] = macro Operators.folded[T, V, Id]
